...
use crate::types::diagnostic::{
    ...
    UNRESOLVED_IMPORT, UNRESOLVED_REFERENCE, UNSUPPORTED_BASE, UNSUPPORTED_OPERATOR,
...
impl<'db, 'ast> TypeInferenceBuilder<'db, 'ast> {
    ...
    fn try_type_call_fallback(&mut self, call_expr: &ast::ExprCall) -> Option<Type<'db>> {
        ...
        // If we have variadic arguments, check if this looks like a type() call that
        // could match either the 1-arg or 3-arg form.
        if has_starred_args || has_kwargs_unpack {
            // If the first argument is a starred expression (e.g., `type(*args)`),
            // we can't determine anything about the arguments. Return type[Unknown].
            if arguments
                .args
                .first()
                .is_some_and(ast::Expr::is_starred_expr)
            {
                // Infer all arguments for side effects.
            ...
            // Check if first arg is a string before inferring all arguments.
            // This avoids conflicts with normal call inference if we decide not to
            // return early.
            if let Some(first_arg) = arguments.args.first() {
                let first_type = self.infer_expression(first_arg, TypeContext::default());
                if first_type.is_assignable_to(self.db(), KnownClass::Str.to_instance(self.db())) {
                    // This looks like a class creation call with ambiguous additional
                    // arguments. Infer remaining arguments and return type[Unknown].
