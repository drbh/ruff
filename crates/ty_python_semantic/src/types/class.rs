...
impl<'db> ClassLiteral<'db> {
    ...
    pub(crate) fn file(self, db: &dyn Db) -> File {
        match self {
            ...
            Self::DynamicNamedTuple(namedtuple) => namedtuple.file(db),
        }
    }
    ...
    pub(crate) fn header_range(self, db: &'db dyn Db) -> TextRange {
        match self {
            ...
            Self::DynamicNamedTuple(namedtuple) => namedtuple.header_range(db),
        }
    }
    ...
    pub(super) fn own_class_member(
        ...
        let (class_literal, specialization) = match self {
            ...
            Self::NonGeneric(ClassLiteral::DynamicNamedTuple(namedtuple)) => {
                return Member {
                    inner: namedtuple.class_member(db, name, MemberLookupPolicy::empty()),
                };
            }
            ...
            for base in self.iter_mro(db, specialization) {
                ...
                match base_class.class_literal(db) {
                    ...
                    ClassLiteral::DynamicNamedTuple(_) => {
                        // NamedTuples cannot define ordering methods in their namespace dict.
                        continue;
                    }
                }
            }
            ...
            && !self
                ...
                .any(|class| match class.class_literal(db) {
                    ClassLiteral::Static(literal) => class_member(db, literal.body_scope(db), name)
                        ...
                        .is_some(),
                    ClassLiteral::Dynamic(literal) => literal
                        .members(db)
                        .iter()
                        .any(|(member_name, _)| member_name.as_str() == name),
...
/// # Salsa interning
///
/// Each `type()` call is uniquely identified by its location in the source code
/// (`file`, `file_scope`, `node_index`), so two different `type()` calls always
/// produce distinct `DynamicClassLiteral` instances, even if they have the
/// same name and bases:
///
/// ```python
/// Foo1 = type("Foo", (Base,), {})
...
pub struct DynamicClassLiteral<'db> {
    ...
    /// The node index of the `type()` call expression, used to uniquely identify
    /// the call site within the file.
